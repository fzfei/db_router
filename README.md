...
- 🎯 Entry points to execute your program using `python -m <db_router>` or `$ db_router` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/fzfei/db_router/branch/main/graph/badge.svg?token=db_router_token_here)](https://codecov.io/gh/fzfei/db_router)
[![CI](https://github.com/fzfei/db_router/actions/workflows/main.yml/badge.svg)](https://github.com/fzfei/db_router/actions/workflows/main.yml)

Awesome db_router created by fzfei

## Install

from source
```bash
git clone https://github.com/fzfei/db_router db_router
cd db_router
make install
```
...
```bash
pip install db_router
```

## Executing

```bash
$ db_router run --port 8080
```

or

```bash
python -m db_router run --port 8080
```

or

```bash
$ uvicorn db_router:app
```
...
```bash
❯ db_router --help
Usage: db_router [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ db_router create-user --help
Usage: db_router create-user [OPTIONS] USERNAME PASSWORD
...
```bash
db_router create-user admin admin --superuser
```
...
Run with `db_router run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `db_router create-user`.
...
-----------------------------------------------------
db_router/__init__.py              4      0   100%
db_router/app.py                  16      1    94%
db_router/cli.py                  21      0   100%
db_router/config.py                5      0   100%
db_router/db.py                   10      0   100%
db_router/models/__init__.py       0      0   100%
db_router/models/content.py       47      1    98%
db_router/routes/__init__.py      11      0   100%
db_router/routes/content.py       52     25    52%
db_router/routes/security.py      15      1    93%
db_router/routes/user.py          52     26    50%
db_router/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from db_router.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in db_router/default.toml.

### As environment variables
```bash
export db_router_KEY=value
export db_router_KEY="@int 42"
export db_router_KEY="@jinja {{ this.db.uri }}"
export db_router_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
db_router_ENV=production db_router run
```
